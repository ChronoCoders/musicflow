...
const AnalyticsDashboard: React.FC<AnalyticsProps> = ({ tracks }) => {
  ...
  // Basic metrics
  const totalRevenue = safeTracks.reduce(
    (sum, track) => sum + track.totalEarnings,
    0,
  );
  const totalTracks = safeTracks.length;
  const avgRevenuePerTrack = totalTracks > 0 ? totalRevenue / totalTracks : 0;
  const totalPayments = safeTracks.reduce(
    (sum, track) => sum + (track.revenues?.length || 0),
    0,
  );
      ...
      .map((track) => ({
        ...
        lastPayment:
          track.revenues && track.revenues.length > 0
            ? new Date(
                track.revenues[track.revenues.length - 1].createdAt,
              ).toLocaleDateString()
            : "No payments",
        avgPerPayment:
          track.revenues && track.revenues.length > 0
    ...
    const data = labels.map((label) => {
      ...
      return safeTracks.reduce((sum, track) => {
        ...
        return (
          ...
          track.revenues
            .filter(
              (rev) => new Date(rev.createdAt).toDateString() === targetDate,
            )
    ...
    safeTracks.forEach((track) => {
      if (track.revenues) {
        track.revenues.forEach((rev) => {
          platformTotals[rev.platform] =
            (platformTotals[rev.platform] || 0) + rev.amount;
        });
      }
    });
  ...
  const getBestDay = () => {
    ...
    const bestDay = Object.entries(dailyTotals).sort(
      ([, a], [, b]) => b - a,
    )[0];
      ...
      <div style={{ display: "flex", gap: "20px", marginBottom: "30px" }}>
        ...
        {bestDay && (
          ...
          >
            <h3>Best Day</h3>
            <p
              style={{ fontSize: "16px", fontWeight: "bold", color: "#FF5722" }}
            >
              {bestDay.amount.toFixed(4)} MATIC
            </p>
            <p style={{ fontSize: "12px", color: "#666" }}>{bestDay.date}</p>
          </div>
        )}
      </div>

...
describe("RoyaltyDistributor - Multi-Owner", function () {
  ...
  it("Should distribute revenue to multiple owners correctly", async function () {
    ...
    const ownerPending = await royaltyDistributor.pendingWithdrawals(owner.address);
    const addr1Pending = await royaltyDistributor.pendingWithdrawals(addr1.address);
